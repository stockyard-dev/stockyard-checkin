...
// Tests for the additive trial-gate middleware on checkin. Booking
...
//   7. End-to-end: tier=="none" → POST /api/members returns 402
//   8. End-to-end: tier=="paid" → POST /api/members returns 200
//   9. End-to-end: tier=="none" → GET /api/members still works (read-through)
...
// checkin's existing limits.go doesn't expose a hook for that.

func newTestDB(t *testing.T) *store.DB {
	t.Helper()
	dir, err := os.MkdirTemp("", "checkin-trial-test-*")
	if err != nil {
...
func newTestServer(t *testing.T, tier string) *Server {
	t.Helper()
	dir, err := os.MkdirTemp("", "checkin-trial-srv-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
...
func TestShouldBlockWrite_NoneTier(t *testing.T) {
	...
	for _, c := range cases {
		req := httptest.NewRequest(c.method, "/api/members", nil)
...
func TestShouldBlockWrite_PaidTier(t *testing.T) {
	...
	for _, m := range []string{"POST", "PUT", "DELETE", "PATCH", "GET"} {
		req := httptest.NewRequest(m, "/api/members", nil)
		if s.shouldBlockWrite(req) {
...
func TestPersistLicense_WritesFile(t *testing.T) {
	dir, err := os.MkdirTemp("", "checkin-persist-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
...
func TestLoadLicenseFromDisk_RoundTrip(t *testing.T) {
	dir, err := os.MkdirTemp("", "checkin-load-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
...
func TestLoadLicenseFromDisk_MissingReturnsEmpty(t *testing.T) {
	dir, err := os.MkdirTemp("", "checkin-missing-test-*")
	if err != nil {
		t.Fatalf("mkdtemp: %v", err)
	}
	defer os.RemoveAll(dir)
